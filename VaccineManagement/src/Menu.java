...
public class Menu implements Serializable {
    ...
    public void add() {
        ...
        while (true) {
            ...
            System.out.println("First letter of vaccine list: A (Astra) / S (SPUTNIK V) /V (Vero Cell) / P (Pfizer) / M (Moderna)!");

            String newInjectionID = input.nextLine();

            if (newInjectionID.contains(" ") || newInjectionID.equals("")) {
                System.out.println("Wrong format input!");
                break;
            }
            ...
            //CheckLastThreeNumbers
            for (int i = 1; i < 4; i++) {
                ch = newInjectionID.charAt(i);
                if (ch >= '0' && ch <= '9') {
                } else {
                    System.out.println("Wrong input format, all 3 last nums expect int!");
                    return;
                }
            }
            ...
            if ( place.equals("") || place.charAt(0) == ' ') {
